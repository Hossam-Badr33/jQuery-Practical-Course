$(document).ready(function () {
  ...
  */
  ...
  let calculatedItem = $(
      $(".calculate-container tr:not(:last-of-type) td:last-child")
    ),
    totalSum = $(".calculate-container tr:last-of-type td:last-child");
  let sum = 0;
  calculatedItem.each(function (index, value) {
    sum += parseInt($(value).text());
  });
  totalSum.text(sum);
  /*autoChange items*/
  //1st soln with setInterval
  let interval2 = setInterval(function () {
    let activeItem = $(".autoChange-container .active-auto");
    activeItem.is(":last-child")
      ? activeItem.fadeOut(200, function () {
          $(this).removeClass("active-auto");
          $(".autoChange-container li:first-child").fadeIn(200, function () {
  ...
  /*create dynamic tab*/
  let activeTab = $(".dynamicTab-item");
  let tabContainer = $(".dynamicTab-container");
  ...
  let horVerButtton = $(".vertical-horizontal");
  horVerButtton.click(function () {
    tabContainer.toggleClass("horizontal vertical");
  });
  /*E-mail suggest box*/
  let suggestInput = $(".suggestBox-text");
  let mails = ["@gmail.com", "@hotmail", "@outlook.com", "@yahoo.com"];
  let mailName = "";
  let result = "";
  suggestInput.on("keyup", function () {
    if (suggestInput.val().includes("@")) {
      // suggestInput.val(suggestInput.val());
    } else {
      mailName = "";
      result = "";
      for (let i = 0; i < mails.length; i++) {
        mailName = $(this).val();
        result =
          result + `<li class="suggestBox-items">${mailName + mails[i]}</li>`;
      }
      $(".suggestBox-choices").html(result);
    }
  });
  $(document).on("click", ".suggestBox-items", function () {
    suggestInput.val($(this).text());
    $(".suggestBox-items").fadeOut(300).remove();
  });
});
